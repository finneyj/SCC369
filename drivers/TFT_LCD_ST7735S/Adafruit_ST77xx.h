...
public:
  ...
  void sendData(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data);

protected:
  uint8_t _colstart = 0,   ///< Some displays need this changed to offset
      _rowstart = 0,       ///< Some displays need this changed to offset
      spiMode = NRF_SPI_MODE_0; ///< Certain display needs MODE3 instead
  ...
  void initSPI(uint32_t freq, uint32_t mode);
